...
The current placeholder services become healthy only after their readiness marker exists. API waits for a healthy database, and Checker waits for a healthy API.
...
That is deliberate: resource names must remain project-scoped so parallel worktrees and CI jobs can use isolated Compose namespaces.

When running multiple worktrees whose directory names may collide, assign a unique project name explicitly:
