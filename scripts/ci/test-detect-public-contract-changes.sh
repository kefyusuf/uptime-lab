...
DETECT="$SCRIPT_DIR/detect-public-contract-changes.sh"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
expect_value "public contract change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base
...
expect_value "detector self-change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "scripts/ci/test-detect-public-contract-changes.sh" '# detector test fixture\n')"
expect_value "detector test change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "scripts/ci/check-public-contract.mjs" '// checker fixture\n')"
expect_value "semantic checker change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "scripts/ci/test-check-public-contract.mjs" '// checker test fixture\n')"
expect_value "semantic checker test change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file ".github/workflows/ci.yml" 'name: CI\n')"
expect_value "workflow change triggers public-contract" "true" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "docs/architecture/unrelated.md" '# Unrelated\n')"
expect_value "unrelated docs skip public-contract" "false" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "apps/api/internal/example.go" 'package example\n')"
expect_value "Go source alone skips public-contract" "false" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base

HEAD="$(commit_file "compose.yaml" 'services: {}\n')"
expect_value "Compose-only change skips public-contract" "false" bash -c "cd '$TMP' && '$DETECT' '$BASE' '$HEAD'"
reset_base
...
printf '\nPublic contract change detection tests: %d passed, %d failed\n' "$PASS" "$FAIL"
test "$PASS" -eq 14
test "$FAIL" -eq 0
